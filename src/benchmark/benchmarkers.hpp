...
template <typename T>
verification::VerificationResult<T> bench_bp_stateful_contains_zero_value_bitwidths(
    const size_t a_count, [[maybe_unused]] const std::string dataset_name) {
  auto decompress_column_a = [](const T *in, T *out,
                                const int32_t value_bit_width,
                                const size_t count) -> void {
  ...
  auto decompress_column_b = [](const T *in, T *out,
                                const int32_t value_bit_width,
                                const size_t count) -> void {
    fls::gpu::bench::query_bp_stateful_contains_zero<T>(in, out, count, value_bit_width,
                                               1);
  };
