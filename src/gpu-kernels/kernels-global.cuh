...
template <typename T, int UNPACK_N_VECTORS, int UNPACK_N_VALUES,
          typename UnpackerT>
__global__ void
query_multicolumn(T *__restrict out, const T *__restrict in_a,
                  const T *__restrict in_b, const T *__restrict in_c,
                  const T *__restrict in_d, 
									const vbw_t value_bit_width_a,
									const vbw_t value_bit_width_b,
									const vbw_t value_bit_width_c,
									const vbw_t value_bit_width_d

									) {
  constexpr uint32_t N_COLUMNS = 4;
  ...
  T registers[N_VALUES * N_COLUMNS];

  auto checker = MagicChecker<T, N_VALUES>(consts::as<T>::MAGIC_NUMBER);

  UnpackerT unpackers[N_COLUMNS] = {
      UnpackerT(in_a + vector_index * utils::get_compressed_vector_size<T>(
						value_bit_width_a), lane, value_bit_width_a, BPFunctor<T>()),
      UnpackerT(in_b + vector_index * utils::get_compressed_vector_size<T>(
						value_bit_width_b), lane, value_bit_width_b, BPFunctor<T>()),
      UnpackerT(in_c + vector_index * utils::get_compressed_vector_size<T>(
						value_bit_width_c), lane, value_bit_width_c, BPFunctor<T>()),
      UnpackerT(in_d + vector_index * utils::get_compressed_vector_size<T>(
						value_bit_width_d), lane, value_bit_width_d, BPFunctor<T>()),
  };

  bool not_all_equal = true;
  for (si_t i{0}; i < mapping.N_VALUES_IN_LANE; i += UNPACK_N_VALUES) {
#pragma unroll
    for (int32_t c{0}; c < N_COLUMNS; ++c) {
      unpackers[c].unpack_next_into(registers + N_VALUES * c);
    }

#pragma unroll
    for (int32_t c{1}; c < N_COLUMNS; ++c) {
#pragma unroll
      for (int va{0}; va < N_VALUES; ++va) {
        not_all_equal &=
            registers[N_VALUES * (c - 1) + va] != registers[N_VALUES * c + va];
      }
    }
  }
