...
get_compressor_manager(const enums_nvcomp::CompressionType compression_type,
...
template <typename T> struct DummyColumn {
  T *in;
  size_t n_values;
};
...
struct DummyDecompressor : flsgpu::device::DecompressorBase<T> {
  ...
  flsgpu::device::BitUnpackerDummy<T, 1, 1, flsgpu::device::BPFunctor<T>>
      unpacker;

  __device__ __forceinline__ DummyDecompressor(const DummyColumn<T> column,
                                               const vi_t vector_index,
                                               const lane_t lane)
      : unpacker(reinterpret_cast<UINT_T *>(
                     column.in + vector_index * consts::VALUES_PER_VECTOR),
                 lane, utils::sizeof_in_bits<T>(),
