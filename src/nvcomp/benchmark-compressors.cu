...
template <typename T>
BenchmarkResult benchmark_thrust(const T *input, const size_t value_count,
  ...
  bool result = thrust::any_of(thrust::device, d_vec.begin(), d_vec.end(),
                               is_equal_to<T>(value_to_search_for));
...
template <typename T>
BenchmarkResult
benchmark_alp(const enums_nvcomp::ComparisonType comparison_type,
              const enums_nvcomp::CompressionType decompressor_enum,
              const T *input, const flsgpu::host::ALPColumn<T> column,
              const T value_to_search_for) {
  bool result = false;
  ...
  case enums_nvcomp::ALP: {
    ...
    if (comparison_type == enums_nvcomp::ComparisonType::DECOMPRESSION) {
      kernels::device::decompress_column<T, UNPACK_N_VECTORS, 1,
                                         ALPDecompressor<T, UNPACK_N_VECTORS>,
                                         flsgpu::device::ALPColumn<T>>
          <<<mapping.n_blocks, mapping.N_THREADS_PER_BLOCK>>>(
              d_column, d_decompression_result.get());
    } else {
      kernels::device::query_column<T, UNPACK_N_VECTORS, 1,
                                    ALPDecompressor<T, UNPACK_N_VECTORS>,
                                    flsgpu::device::ALPColumn<T>>
          <<<mapping.n_blocks, mapping.N_THREADS_PER_BLOCK>>>(
  ...
  case enums_nvcomp::GALP: {
    flsgpu::host::ALPExtendedColumn<T> column_extended =
        column.create_extended_column();
    flsgpu::device::ALPExtendedColumn<T> d_column =
        column_extended.copy_to_device();
    stopwatch.start();
    if (comparison_type == enums_nvcomp::ComparisonType::DECOMPRESSION) {
      kernels::device::decompress_column<
          T, UNPACK_N_VECTORS, 1, ALPExtendedDecompressor<T, UNPACK_N_VECTORS>,
          flsgpu::device::ALPExtendedColumn<T>>
          <<<mapping.n_blocks, mapping.N_THREADS_PER_BLOCK>>>(
              d_column, d_decompression_result.get());
    } else {
      kernels::device::query_column<
          T, UNPACK_N_VECTORS, 1, ALPExtendedDecompressor<T, UNPACK_N_VECTORS>,
          flsgpu::device::ALPExtendedColumn<T>>
          <<<mapping.n_blocks, mapping.N_THREADS_PER_BLOCK>>>(
              d_column, d_query_result.get(), value_to_search_for);
  ...
  if (comparison_type == enums_nvcomp::ComparisonType::DECOMPRESSION) {
    ...
    kernel_successful = check_if_device_buffers_are_equal<T>(
        d_decompression_result.get(), d_input.get(), column.get_n_values());
  }
...
template <typename T>
BenchmarkResult
benchmark_hwc(const enums_nvcomp::ComparisonType comparison_type,
  ...
  if (comparison_type == enums_nvcomp::ComparisonType::DECOMPRESSION_QUERY) {
    hwc::DummyColumn<T> d_column{reinterpret_cast<T *>(d_output_buffer),
                                 value_count};
    kernels::device::query_column<
        T, UNPACK_N_VECTORS, 1, hwc::DummyDecompressor<T>, hwc::DummyColumn<T>>
        <<<mapping.n_blocks, mapping.N_THREADS_PER_BLOCK>>>(
            d_column, d_query_result.get(), value_to_search_for);
  }

  double execution_time_ms = stopwatch.stop();
  CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
benchmark_thrust<float>(const float *input, const size_t value_count,
                        const float value_to_search_for);
template BenchmarkResult
benchmark_alp<float>(const enums_nvcomp::ComparisonType comparison_type,
                     const enums_nvcomp::CompressionType decompressor_enum,
                     const float *input, const size_t value_count,
                     const float value_to_search_for);
template BenchmarkResult
benchmark_hwc<float>(const enums_nvcomp::ComparisonType comparison_type,
                     const enums_nvcomp::CompressionType compression_type,
                     const float *input, const size_t value_count,
                     const float value_to_search_for);

template BenchmarkResult
benchmark_thrust<double>(const double *input, const size_t value_count,
                         const double value_to_search_for);
template BenchmarkResult
benchmark_alp<double>(const enums_nvcomp::ComparisonType comparison_type,
                      const enums_nvcomp::CompressionType decompressor_enum,
                      const double *input, const size_t value_count,
                      const double value_to_search_for);
template BenchmarkResult
benchmark_hwc<double>(const enums_nvcomp::ComparisonType comparison_type,
                      const enums_nvcomp::CompressionType compression_type,
                      const double *input, const size_t value_count,
                      const double value_to_search_for);
