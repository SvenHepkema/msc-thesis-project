...
template <typename T> int run_verification(CLIArgs args) {
  ...
  if (results.size() == 0) {
		if (args.print_debug) fprintf(stderr, "Compression successful.\n");
    return 0;
  }

	if (args.print_debug) 
  for (auto result : results) {
    fprintf(stderr, "Value bit width %d failed.\n", result.first);
...
int main(int argc, char **argv) {
  ...
  if (args.use_signed) {
    switch (args.lane_width) {
			/*
    case 64: {
      run_verification<int64_t>(args);
      break;
    }*/
    case 32: {
      run_verification<int32_t>(args);
      break;
    }/*
    case 16: {
      run_verification<int16_t>(args);
      break;
    }
    case 8: {
      run_verification<int8_t>(args);
      break;
    }*/
    }
  } else {
    switch (args.lane_width) {
			/*
    case 64: {
      run_verification<uint64_t>(args);
      break;
    }*/
    case 32: {
      run_verification<uint32_t>(args);
      break;
    }/*
    case 16: {
      run_verification<uint16_t>(args);
      break;
    }
    case 8: {
      run_verification<uint8_t>(args);
      break;
    }*/
    }
  }
