...
unpack_vector(const T *__restrict in, T *__restrict out, const uint16_t lane,
  ...
  constexpr uint8_t LANE_BIT_WIDTH = utils::get_lane_bitwidth<T>();
  ...
  T value_mask = utils::set_first_n_bits<T>(value_bit_width);

  T line_buffer[UNPACK_N_VECTORS];
...
struct MultiVecScanner {
  const T *in;
  T line_buffers[UNPACK_N_VECTORS];
  uint8_t counter = 0;
  uint8_t value_bit_width;

  __device__ MultiVecScanner(const T *a_in, const uint8_t a_value_bit_width,
  ...
  __device__ __forceinline__ void _unpack_value(T *__restrict out,
                                                const uint16_t start_index) {
    constexpr uint8_t LANE_BIT_WIDTH = utils::sizeof_in_bits<T>();
    constexpr uint32_t N_LANES = utils::get_n_lanes<T>();
    uint16_t preceding_bits = (start_index * value_bit_width);
    uint16_t buffer_offset =
        preceding_bits % LANE_BIT_WIDTH; // Make this a class var
    T value_mask = utils::set_first_n_bits<T>(value_bit_width);

    T buffer_offset_mask;

    int32_t encoded_vector_offset = utils::get_compressed_vector_size<T>(
        value_bit_width); // can be a class var

    bool line_buffer_is_empty = buffer_offset == 0;
    if (line_buffer_is_empty) {
#pragma unroll
      for (int i = 0; i < UNPACK_N_VECTORS; ++i) {
        line_buffers[i] = *(in + i * encoded_vector_offset);
      }
      in += N_LANES;
    }
...
#pragma unroll
        for (int v = 0; v < UNPACK_N_VECTORS; ++v) {
          line_buffers[v] = *(in + v * encoded_vector_offset);
        }
        in += N_LANES;
        buffer_offset -= LANE_BIT_WIDTH;
      }
...
#pragma unroll // TODO: remove value buffer, write directly to out, assume lane
               // buffer
      for (int i = 0; i < UNPACK_N_VECTORS; ++i) {
